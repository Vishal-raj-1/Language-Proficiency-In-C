...
struct word get_word(char* text, int beg, int end) {
    ...
    int index = 0;
    int i;
    for (i = beg; i <= end; i++) answer.data[index++] = text[i];
    answer.data[index] = 0;
    return answer;
}
...
struct sentence get_sentence(char* text, int beg, int end) {
    ...
    answer.word_count = 1;
    int i;
    for (i = beg; i <= end; i++) if (text[i] == ' ') ++answer.word_count;
    answer.data = calloc(answer.word_count, sizeof(struct word));
    int start = beg;
    int index = 0;
    for (i = beg; i <= end; i++) if (text[i] == ' ') {
...
struct paragraph get_paragraph(char* text, int beg, int end) {
    ...
    int i;
    for (i = beg; i <= end; i++) if (text[i] == '.') ++answer.sentence_count;
